...
def rnn_encoder(rnn_type, x, mask, d_dim, n_layer, initializer, pooling=None):
    ...
    with tf.compat.v1.variable_scope('rnn_{}'.format(0)):
...
def dynamic_max_pooling(x, mask):
    ...
    return tf.reduce_max(input_tensor=x + mask * 999999, axis=1)
...
def rnn_encoder_single_layer(rnn_type, input, seq_len, d_dim, initializer):
    ...
    with tf.compat.v1.variable_scope('rnn_encoder'):
        batch_size = tf.shape(input=seq_len)[0]
        cell = {}
        initial_state = {}
        for d in ['forward', 'backward'] if bi_directional else ['forward']:
            with tf.compat.v1.variable_scope(d):
                ...
                initial_state[d] = tf.nn.rnn_cell.LSTMStateTuple(c_states,
                                                                 h_states)

        if bi_directional:
            raw_outputs, (fw, bw) = \
                tf.compat.v1.nn.bidirectional_dynamic_rnn(cell['forward'], cell['backward'], input, dtype=tf.float32, sequence_length=seq_len, initial_state_fw=initial_state['forward'], initial_state_bw=initial_state['backward'])
            raw_outputs = tf.concat(raw_outputs, axis=2)
            final_states = tf.concat([fw.h, bw.h], axis=1)

        else:
            raw_outputs, final_states = \
                tf.compat.v1.nn.dynamic_rnn(cell['forward'], input, dtype=tf.float32, sequence_length=seq_len, initial_state=initial_state['forward'])
...
def ffnn(inputs, h_dim, out_dim, n_layer, initializer,
    ...
    with tf.compat.v1.variable_scope('{}_{}'.format(scope_name, n_layer-1)):
        W = tf.compat.v1.get_variable('W',
                            shape=[h_dim, out_dim],
                            initializer=initializer)
        if len(inputs.get_shape().as_list()) == 3:
            W = tf.tile(tf.expand_dims(W, axis=0), [tf.shape(input=inputs)[0], 1, 1])
        y_raw = tf.matmul(h, W)
        if use_bias:
            b = tf.compat.v1.get_variable('b',
                                shape=[out_dim],
...
def create_mixed_trainable_emb(dim, n_ws, n_special_ws, initializer,
    ...
    with tf.compat.v1.variable_scope(scope_name):
        pad_e = tf.compat.v1.get_variable(
            "pad_e",
            dtype=tf.float32,
            shape=[1, dim],
            initializer=tf.compat.v1.zeros_initializer(),
        ...
        special_e = tf.compat.v1.get_variable(
            "special_e",
            dtype=tf.float32,
...
class BaseModel:
    def create_holder(self):
        self.x = tf.compat.v1.placeholder(tf.int32, [None, None, None])
        self.x_mask = tf.compat.v1.placeholder(tf.int32, [None, None, None])
        self.xw = tf.compat.v1.placeholder(tf.int32, [None, None])
        self.xw_mask = tf.compat.v1.placeholder(tf.int32, [None, None])

        self.y = tf.compat.v1.placeholder(tf.int32, [None, None])
        self.drp_keep = tf.compat.v1.placeholder(tf.float32)
    ...
    def get_hybrid_emb(self, x, x_mask, xw, ce, we, initializer):
        with tf.compat.v1.variable_scope('hybrid_emb'):
            dims = shape_list(x)
            ...
            x_rep = tf.nn.embedding_lookup(params=ce, ids=x_flat)

            with tf.compat.v1.variable_scope('c_encoder'):
            ...
            xw_rep = tf.nn.embedding_lookup(params=we, ids=xw_fat)
    ...
    def encode(self, x, x_mask, xw, xw_mask,
        ...
        with tf.compat.v1.variable_scope('encoder', reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('main_text'):
                w_rep, _ = self.get_hybrid_emb(x, x_mask, xw,
                                               ce, we, initializer)
                w_rep = tf.nn.dropout(w_rep, rate=1 - (drp_keep))

            with tf.compat.v1.variable_scope('w_encoder'):
    ...
    def hw_pooling(self, hw, mask, hw_size, out_size, use_l2, initializer,
        ...
        with tf.compat.v1.variable_scope(scope_name, reuse=reuse):
            ...
            with tf.compat.v1.variable_scope('ff0', reuse=reuse):
                h = ffnn(
                        h, hw_size, out_size,
                        1, initializer)
            if use_l2:
